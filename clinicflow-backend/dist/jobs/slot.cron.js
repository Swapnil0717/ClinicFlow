...
const slot_service_1 = require("../modules/slots/slot.service");
...
node_cron_1.default.schedule("0 0 * * *", async () => {
    ...
    for (const rs of recurringSlots) {
        for (let i = 0; i < futureDays; i++) {
            ...
            if (currentDate < rs.validFrom ||
                currentDate > rs.validTill)
                continue;
            if (!rs.daysOfWeek.includes(day))
                continue;
            const date = new Date(currentDate.toISOString().split("T")[0] + "T00:00:00.000Z");
            ...
            const subSlots = slot_service_1.SlotService.generateSubSlots({
                slotId: slot.id,
